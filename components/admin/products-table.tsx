    ...
    <div className="space-y-4">
      ...
      <div className="glass rounded-lg p-4 flex flex-wrap gap-3">
        ...
        >
          ...
          {["AVAILABLE", "ON_REQUEST", "OUT_OF_STOCK"].map((s) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-carbon-800">
              {products.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center text-carbon-500 py-10">No products found</td>
                </tr>
              ) : (
                products.map((product) => (
                  <tr key={product.id} className="hover:bg-carbon-800/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={cn("text-xs px-2 py-0.5 rounded-sm", statusColors[product.status])}>
                        {product.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-carbon-400 text-xs">
                      {product.brands.slice(0, 2).join(", ")}{product.brands.length > 2 ? "..." : ""}
                    </td>
                    <td className="px-4 py-3 text-carbon-500 text-xs">{formatDate(product.createdAt)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link href={`/admin/products/${product.id}/edit`} className="text-carbon-400 hover:text-gold transition-colors">
                          <Edit size={14} />
                        </Link>
                        <button
